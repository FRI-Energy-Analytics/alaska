...
class Params:
    ...
    hidden_size: int = (
        150  # of the encoder; default decoder size is doubled if encoder is bidi
    )
    ...
    pointer: bool = (
        True  # use pointer network (copy mechanism) in addition to word generator?
    )
    ...
    cover_func: str = (
        "max"  # how to aggregate previous attention distributions? sum or max
    )
    cover_loss: float = (
        1  # add coverage loss if > 0; weight of coverage loss as compared to NLLLoss
    )
    show_cover_loss: bool = (
        False  # include coverage loss in the loss shown in the progress bar?
    )
    ...
    partial_forcing: bool = (
        True  # in a seq, can some steps be teacher forced and some not?
    )
    forcing_decay_type: Optional[str] = "exp"  # linear, exp, sigmoid, or None
    forcing_decay: float = 0.9999
    sample: bool = (
        True  # are non-teacher forced inputs based on sampling or greedy selection?
    )
    ...
    rl_ratio_power: float = (
        1  # increase rl_ratio by **= rl_ratio_power after each epoch; (0, 1]
    )
    rl_start_epoch: int = (
        1  # start RL at which epoch (later start can ensure a strong baseline)?
    )
    ...
    truncate_src: bool = (
        True  # truncate to max_src_len? if false, drop example if too long
    )
    truncate_tgt: bool = (
        True  # truncate to max_tgt_len? if false, drop example if too long
    )

    # Saving model automatically during training
    model_path_prefix: Optional[str] = "weights"
    keep_every_epoch: bool = (
        False  # save all epochs, or only the best and the latest one?
    )
