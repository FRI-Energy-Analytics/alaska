...
def eval_bs(test_set: Dataset, vocab: Vocab, model: Seq2Seq, params: Params):
  ...
  for i in prog_bar:
    batch = next(test_gen)
    file_content = eval_bs_batch(batch, model, vocab, pack_seq=params.pack_seq,
...
if __name__ == "__main__":
  ...
  parser = argparse.ArgumentParser(description='Evaluate the AlasKA model.')
